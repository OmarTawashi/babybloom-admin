...
import { FileText, Edit3, Trash2, Plus, X, Search, RefreshCw } from 'lucide-react'
...
const categoryEmoji = {
  Onboarding: '🎉', Sleep: '😴', Feeding: '🍼', AI: '🤖', Family: '👨‍👩‍👧‍👦',
  Insights: '📊', Billing: '💳', Legal: '⚖️', General: '📄',
}
...
function ArticleEditor({ article, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast({ type: 'error', title: 'Failed to save article' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50">
      <div className="fixed inset-0 bg-black/40 glass" onClick={onClose} />
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-2xl bg-surface shadow-2xl overflow-auto animate-slide-in-right">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold">{article?.id ? 'Edit Article' : 'New Article'}</h2>
              <p className="text-sm text-text-secondary mt-0.5">Fill in the details below</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-border">
              <button type="button" onClick={onClose} className="px-4 py-2.5 text-sm font-medium rounded-xl border border-border hover:bg-cream transition-colors">Cancel</button>
              <button type="submit" disabled={saving} className="px-6 py-2.5 bg-coral text-white rounded-xl text-sm font-semibold hover:bg-coral-dark disabled:opacity-50 transition-colors shadow-sm">
...
export default function Content() {
  ...
  const fetchArticles = useCallback(async () => {
    ...
    try {
      ...
      toast({ type: 'error', title: 'Failed to load articles' })
    } finally {
      setLoading(false)
    }
  ...
  const handleDelete = async (article) => {
    try {
      ...
      toast({ type: 'success', title: 'Article deleted', message: article.title })
      fetchArticles()
    } catch {
      toast({ type: 'error', title: 'Failed to delete article' })
    }
    setConfirm(null)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={fetchArticles} className="p-2.5 rounded-xl hover:bg-cream transition-colors" title="Refresh">
            <RefreshCw size={16} className="text-text-secondary" />
          </button>
          <button onClick={() => setEditing({})} className="flex items-center gap-2 px-4 py-2.5 bg-coral text-white rounded-xl text-sm font-semibold hover:bg-coral-dark transition-colors shadow-sm">
        ...
        <>
          {articles.length === 0 ? (
            <EmptyState
              ...
              description={search || statusFilter ? 'Try adjusting your filters' : 'Create your first article to get started'}
            />
          ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-cream/30">
                      ...
                      <th className="text-left text-xs font-semibold text-text-secondary uppercase tracking-wider px-6 py-3">Access</th>
                  ...
                  <tbody>
                    {articles.map((article) => (
                      <tr key={article.id} className="border-b border-border table-row-hover">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span className="text-base">{article.emoji || categoryEmoji[article.category] || '📄'}</span>
                            <div>
                              <span className="font-medium text-sm">{article.title}</span>
                              {article.summary && <p className="text-xs text-text-secondary truncate max-w-xs">{article.summary}</p>}
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${article.is_published ? 'bg-emerald-500/10 text-emerald-600' : 'bg-mango/10 text-amber-600'}`}>
                            {article.is_published ? 'Published' : 'Draft'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          {article.is_premium
                            ? <span className="px-2 py-0.5 bg-plum/10 text-plum rounded text-xs font-semibold">Premium</span>
                            : <span className="text-xs text-text-secondary">Free</span>
                          }
                        </td>
                        <td className="px-6 py-4 text-sm text-text-secondary">
                          {new Date(article.updated_at).toLocaleDateString('en', { month: 'short', day: 'numeric' })}
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-0.5">
                            <button onClick={() => setEditing(article)} className="p-1.5 rounded-lg hover:bg-cream transition-colors" title="Edit">
                              <Edit3 size={14} className="text-text-secondary" />
                            </button>
                            <button onClick={() => setConfirm(article)} className="p-1.5 rounded-lg hover:bg-red-500/10 transition-colors" title="Delete">
